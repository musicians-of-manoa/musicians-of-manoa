...
const GuitaristsPage = async ({ searchParams }: CategoryPageProps) => {
  ...
  const profiles: ProfileWithReviews[] = await prisma.profile.findMany({
    where: { instruments: { contains: 'Guitar' } },
    skip,
        ...
        <Row>
          <Col>
            <Col className="text-center py-4" style={{ fontFamily: 'Arial' }}>
              <h2>
                <strong>Discover Guitarists</strong>
              </h2>
            </Col>
          </Col>
        </Row>
